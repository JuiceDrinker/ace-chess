...
fn parse_moves(input: &str) -> IResult<&str, &str, ErrorTree<&str>> {
    alt((parse_move_white, parse_move_black))(input)
}

fn parse_variations(input: &str) -> IResult<&str, ParsedMove, ErrorTree<&str>> {
    dbg!("variations", input);
    delimited(tag("("), parse_move_with_comment, tag(")"))(input)
}
fn parse_move_with_comment(input: &str) -> IResult<&str, ParsedMove, ErrorTree<&str>> {
    let (rest, parsed_move) = parse_moves(input)?;
    let (rest, comment) = (opt(parse_comments))(rest.trim_start())?;
    dbg!("ifeoaf", rest);
    let (_, variations) = (parse_variations)(rest)?;
    dbg!(variations);

    Ok((rest, ParsedMove::new(parsed_move, comment)))
}
...
mod test {
    ...
    fn move_with_comment() {
        let (a, b) = parse_move_with_comment("1.e4 {This is a comment}(1... e5 2.d4 )").unwrap();
        dbg!((a, b));
    }
